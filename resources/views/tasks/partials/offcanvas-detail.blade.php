...
<div class="offcanvas offcanvas-end show shadow" tabindex="-1" id="taskDetailOffcanvas" style="width: 600px; max-width: 95vw;" data-bs-backdrop="true">
    <div class="offcanvas-header border-bottom bg-light py-3">
        <div class="d-flex align-items-center gap-2">
            ...
            @php
                $isTaskOwner = (($activeTask['owner_id'] ?? 0) === ($currentUser['id'] ?? 0)) || (($currentUser['role'] ?? '') === 'admin');
            @endphp
            @if($isTaskOwner)
            ...
            @else
                <div class="d-inline-flex align-items-center gap-1 text-muted" style="font-size: 0.75rem;" title="List tempat task ini berada">
                    <i class="bi bi-folder2 text-secondary" style="font-size: 0.85rem;"></i>
                    <span class="fw-semibold">{{ $lists[$activeTask['list_id']]['name'] ?? 'List' }}</span>
                </div>
            @endif
        </div>
        <div class="d-flex align-items-center gap-2">
            @if($isTaskOwner)
                <!-- Tombol Hapus Task (SRS-F-02 - Novelya; SRS-F-19 - Anggota 2) -->
    ...
    <div class="offcanvas-body p-4">
        ...
        <div class="d-flex align-items-start justify-content-between gap-2 mb-2">
            ...
            @else
                <span class="badge bg-light text-muted border" style="font-size: 0.7rem;" title="Hanya Pemilik Task yang berwenang mengubah rincian task (SRS-F-19)">
                    <i class="bi bi-lock-fill me-1"></i> Read-Only (Non-Owner)
                </span>
            @endif
        </div>
            ...
            <div class="collapse mb-3" id="editTaskCollapse">
                <div class="card card-body bg-light border p-3 rounded-3">
                    ...
                    <form action="{{ route('tasks.update', $activeTask['id']) }}" method="POST">
                        ...
                        <div class="mb-2">
                            <label class="form-label small fw-semibold mb-1">Judul Task <span class="text-danger">*</span></label>
                            <input type="text" name="title" class="form-control form-control-sm" value="{{ $activeTask['title'] }}" required minlength="1" maxlength="255">
                        </div>
                        <div class="mb-2">
                            <label class="form-label small fw-semibold mb-1">Deskripsi</label>
                            <textarea name="description" class="form-control form-control-sm" rows="2" maxlength="1000">{{ $activeTask['description'] }}</textarea>
                        </div>
                        <div class="row g-2 mb-2">
                            <div class="col-6">
                                <label class="form-label small fw-semibold mb-1">Prioritas (SRS-F-05) <span class="text-danger">*</span></label>
                            ...
                            <div class="col-6">
                                <label class="form-label small fw-semibold mb-1">Deadline (SRS-F-06) <span class="text-danger">*</span></label>
                                <input type="date" name="deadline" class="form-control form-control-sm" value="{{ $activeTask['deadline'] }}" min="2020-01-01" max="2099-12-31" required>
                            </div>
                        </div>
